...
(function(){
  ...
  let spineItems = [];   // epub 章节（spine）列表，用于章节间切换
  let spinePos = 0;      // 当前 spine 章节索引
  ...
  function handleFile(file){
    ...
    const type = isEpub?TYPES.EPUB:TYPES.TXT;
    // 同名书覆盖：已有同书名同类型则确认后替换（清进度）
    const existing = library.find(b=> b.title===title && b.type===type);
    if(existing){
      if(!confirm('已存在《'+title+'》，覆盖原书并清除阅读进度？')) return;
      idbDel(existing.id);
      library = library.filter(b=> b.id!==existing.id);
    }
    const id = 'b_' + Date.now() + '_' + Math.random().toString(36).slice(2,8);
    const entry = { id, title, type, progress:0, progressText:'未开始' };
    library.push(entry);

    if(isTxt){
      // 把正文存进本地（IDB），重开网页才能恢复阅读进度
      idbSet(id, file).then(()=>{
  ...
  /* ---------- epub 封面提取 ---------- */
  function extractCover(book, file){
      ...
      try{
        ...
        b.coverUrl().then(url=>{
          ...
          fetch(url).then(r=>r.blob()).then(blob=>{
            ...
            fr.onload = ()=>{ book.cover = fr.result; persist(); renderLibrary(); };
  ...
  function persist(){
    ...
    const slim = library.map(b => ({
      ...
      progressIdx:b.progressIdx, progressCfi:b.progressCfi, cover:b.cover
    }));
  ...
  function renderLibrary(){
    ...
    library.forEach(book=>{
      ...
      card.innerHTML =
        '<div class="thumb">'+ (book.cover ? '<img src="'+book.cover+'" alt="">' : typeLbl) +'</div>'+
        '<div class="meta">'+
    ...
    idbGet(book.id).then(blob=>{
      ...
      blob.arrayBuffer().then(buf=>{
        try{
          ...
          spineItems = (currentBook.spine && currentBook.spine.spineItems) ? currentBook.spine.spineItems.slice() : [];
          spinePos = 0;
          ...
          currentRendition.on('relocated', (loc)=>{
            ...
            const s = loc.start || {};
            // 定位当前章节
            if(typeof s.index === 'number'){ spinePos = s.index; }
            else if(s.href){ spineItems.forEach((it,i)=>{ if(it.href && (it.href===s.href || s.href.indexOf(it.href)>=0)) spinePos=i; }); }
            if(s.cfi){
              current.progressCfi = s.cfi;
  ...
  function epubNext(){
    ...
    if(!spineItems.length){ try{ currentRendition.next(); }catch(e){} return; }
    if(spinePos < spineItems.length-1){
      spinePos++;
      const it = spineItems[spinePos];
      currentRendition.display(it.href || it.cfi);
    } else { toast('已是最后一章'); }
  }
  function epubPrev(){
    if(!currentRendition){ return; }
    if(!spineItems.length){ try{ currentRendition.prev(); }catch(e){} return; }
    if(spinePos > 0){
      spinePos--;
      const it = spineItems[spinePos];
      currentRendition.display(it.href || it.cfi);
  ...
  function init(){
    ...
    $('#btn-settings').addEventListener('click', ()=> toggleSheet('settings'));
